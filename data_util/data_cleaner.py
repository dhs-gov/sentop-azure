...
def remove_invalid_datapoints(row_id_list, data_list, all_stop_words):
    ...
    sentlog.append("----------------------------")
    sentlog.append("Removing invalid documents (data points).")
    ...
    for i in range (len(data_list)):
        ...
        if not text_preproc or text_preproc == 'na' or text_preproc == 'n/a' or text_preproc == 'not applicable':
            sentlog.append(f"- [ID]: {id}, [TEXT]: '{text}', [ERROR]: 'NA', 'N/A', or 'Not Applicable' found.")
        ...
        if not found_letter:
            sentlog.append(f"- [ID]: {id}, [TEXT]: '{text}', [ERROR]: No alphabetic characters found.")
        ...
        if not good_num:
            sentlog.append(f"- [ID]: {id}, [TEXT]: '{text}', [ERROR]: Number of valid (alphabetic) words less than min allowed after preprocessing.")
        ...
        if not new_text:
            sentlog.append(f"- [ID]: {id}, [TEXT]: '{text}', [ERROR]: Text matches a stop word.")
        ...
        if not new_text:
            sentlog.append(f"- [ID]: {id}, [TEXT]: '{text}', [ERROR]: Text comprises all stop words.")
        ...
        # Check if text comprises all stop words 
        if not cleaned_text:
            sentlog.append(f"- [ID]: {id}, [TEXT]: '{text}', [ERROR]: Cleaned text removed all words.")
    ...
    sentlog.append(f"\nNum docs to be analyzed: {total_docs_to_analyze} out of {len(data_list)} non-blank highlighted docs({total_perc_ignored}%).")
    sentlog.append("----------------------------------------------------------")

...
class Sentiment:
    def __init__(self, sentiment, nothate, hate):
        self.sentiment = sentiment
        self.nothate = float(truncate(nothate, 3))
        self.hate = float(truncate(hate, 3))
...
def get_sentiment(classifier, text):
    ...
    with globalutils.suppress_stdout_stderr():
        ...
        confidence_scores = classifier.tag_text(
            ...
            model_name_or_path="cardiffnlp/twitter-roberta-base-hate",
            mini_batch_size=1
        )
...
def assess(classifier, docs):
    ...
    sentlog.append("----------------------------------------------------------")
    sentlog.append("Assessing hate sentiment. Please wait...")
    sentiments = []
    i = 0
    for doc in docs:
        #print("doc: ", doc)
